...
document.addEventListener("DOMContentLoaded", () => {
  ...
  <span class="terminal-command">clear</span>      - Clear terminal output`,
    ...
    whoami:
      () => `<span class="terminal-highlight">${resumeData.basics.name}</span>
...
[ ${resumeData.skills.join(" | ")} ]`,
  ...
  function printToTerminal(text, isCommand = false, rawCmd = "") {
    ...
    document
      ...
      .scrollTo({
        top: document.querySelector(".terminal-window").scrollHeight,
        behavior: "smooth",
      });
  }
    ...
    if (e.key === "Enter") {
      ...
      if (rawCmd) {
        ...
        if (commands[cmd]) {
          ...
          setTimeout(() => inputLine.classList.remove("shake"), 400);
        }
      }
    }
  });
  ...
  function generateBotResponse(query) {
    ...
    if (q.includes("google")) {
      const googleExp = resumeData.experience.find(
        (e) => e.company === "Google",
      );
    ...
    if (q.includes("splunk") || q.includes("cisco")) {
      const splunkExp = resumeData.experience.find(
        (e) => e.company === "CISCO (Splunk)",
      );
    ...
    if (q.includes("project") || q.includes("built")) {
      return (
        "Some of my key projects include:<br><ul>" +
        resumeData.projects
          .map(
            (p) => `<li><b>${p.name} (${p.context}):</b> ${p.description}</li>`,
          )
          .join("") +
        "</ul>"
      );
  ...
  const bootLines = [
    ...
    "<br>Welcome to Anand Suthar's interactive portfolio.",
  ...
  function printBootLine() {
    if (bootIndex < bootLines.length) {
      printToTerminal(bootLines[bootIndex]);
      bootIndex++;
